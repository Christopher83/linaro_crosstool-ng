    ...
    case "${CT_BARE_METAL},${CT_CANADIAN},${CT_THREADS}" in
        y,*,*)
            if [ "${CT_STATIC_TOOLCHAIN}" = "y" ]; then
                do_cc_core mode=baremetal build_libgcc=yes build_libstdcxx=yes build_libgfortran=yes build_staticlinked=yes build_manuals=yes
            else
                do_cc_core mode=baremetal build_libgcc=yes build_libstdcxx=yes build_libgfortran=yes build_manuals=yes
            fi
            ;;
        ,y,*)   ;;
        ,,nptl)
...
#  - we need to build libfortran or not         : build_libgfortran=[yes|no]   (default: no)
...
do_cc_core() {
    ...
    # When have prebuilt libc, skip this step.
    [ "${CT_LIBC_prebuilt}" = "y" ] && return
    ...
    for tmp in ARCH ABI CPU TUNE FPU; do
