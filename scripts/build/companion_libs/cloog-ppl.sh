...
do_cloog_ppl_get() { :; }
do_cloog_ppl_extract() { :; }
do_cloog_ppl() { :; }

# Overide functions depending on configuration
if [ "${CT_CLOOG_PPL}" = "y" ]; then

# Download CLooG
do_cloog_ppl_get() {
    CT_GetFile "cloog-ppl-${CT_CLOOG_PPL_VERSION}"  \
...
do_cloog_ppl_extract() {
    ...
    CT_Extract "cloog-ppl-${CT_CLOOG_PPL_VERSION}"
    ...
    case "${CT_CLOOG_PPL_VERSION}" in
        0.15.3) _t="";;
        *)      _t="-${CT_CLOOG_PPL_VERSION}";;
    esac
    CT_Pushd "${CT_SRC_DIR}/cloog-ppl${_t}"
    CT_Patch nochdir "cloog-ppl" "${CT_CLOOG_PPL_VERSION}"
    if [ "${CT_CLOOG_PPL_VERSION}" = "0.15.10" ]; then
